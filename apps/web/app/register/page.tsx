...
export default function Register() {
    ...
    const handleLogin = (provider: "google" | "twitter") => {
        window.location.href = `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api/v1"}/auth/${provider}`;
    };
        ...
        <div className="min-h-screen bg-[#09090b] flex flex-col justify-center items-center relative overflow-hidden">
            ...
            <div className="z-10 w-full max-w-sm px-6">
                ...
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-2xl font-bold text-white mb-2 tracking-tight">{t('auth_join')}</h1>
                    <p className="text-[#a1a1aa] text-sm">{t('auth_journey')}</p>
                </div>

                <div className="bg-[#09090b] border border-[#27272a] p-8 shadow-sm space-y-4">
                    <button
                        onClick={() => handleLogin("google")}
                        className="w-full bg-[#18181b] border border-[#27272a] text-white py-3 flex items-center justify-center gap-3 hover:bg-[#27272a] transition-all font-medium text-sm"
                    >
